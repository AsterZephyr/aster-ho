import type { BaselineKey, BaselineStore, AnomalyResult } from "@ho/baseline";
...
export function evaluate(state: WindowState, condition: AlertCondition, ruleName: string, now: number): AlertEvent | undefined {
	if (condition.filter) {
...
export function evaluateAnomaly(
	...
	const result: AnomalyResult = store.isAnomaly(key, condition.metric, value, condition.zscoreThreshold);
