...
describe("E2BSandboxReceiver", () => {
	...
	it("creates span for exec event", () => {
		...
		receiver.ingest({ sandbox_id: "sbx-1", action: "exec", command: "npm test", exit_code: 0, duration_ms: 3000, started_at: "", completed_at: "" });
	...
	it("creates span for file operations", () => {
		const receiver = new E2BSandboxReceiver();
		receiver.init(trace.getTracer("test"));
		receiver.ingest({ sandbox_id: "sbx-1", action: "write_file", path: "/app/main.py", duration_ms: 50, started_at: "", completed_at: "" });
	...
	it("sets error on failed exec", () => {
		const receiver = new E2BSandboxReceiver();
		receiver.init(trace.getTracer("test"));
		receiver.ingest({ sandbox_id: "sbx-1", action: "exec", command: "make", exit_code: 1, duration_ms: 100, started_at: "", completed_at: "" });
	...
	it("sets error from error field", () => {
		const receiver = new E2BSandboxReceiver();
		receiver.init(trace.getTracer("test"));
		receiver.ingest({ sandbox_id: "sbx-1", action: "exec", command: "x", duration_ms: 0, error: "Sandbox timeout", started_at: "", completed_at: "" });
	...
	it("includes duration_ms attribute", () => {
		const receiver = new E2BSandboxReceiver();
		receiver.init(trace.getTracer("test"));
		receiver.ingest({ sandbox_id: "sbx-2", action: "exec", command: "ls", exit_code: 0, duration_ms: 42, started_at: "", completed_at: "" });
